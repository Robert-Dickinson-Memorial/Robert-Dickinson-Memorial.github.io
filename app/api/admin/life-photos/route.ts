...
export async function POST(request: Request) {
  ...
  const { lifePhotos } = await getSiteContent();
  ...
  const photo: LifePhoto = { id: photoId, objectKey,
    ...
    alt: (form.has("alt") ? clean(form.get("alt"), 500) : previous?.alt) || "Early-life photograph of Robert Dickinson" };
...
export async function PATCH(request: Request) {
  const denied = await authorize(request); if (denied) return denied;
  const body = await request.json() as { id?: string; caption?: string; date?: string; alt?: string };
  const { lifePhotos } = await getSiteContent();
  if (!lifePhotos.some((photo) => photo.id === body.id)) return Response.json({ error: "Photo not found." }, { status: 404 });
  const photos = lifePhotos.map((photo) => photo.id === body.id ? { ...photo, caption: clean(body.caption, 2000), date: clean(body.date, 100), alt: clean(body.alt, 500) } : photo);
  await save(photos);
  return Response.json({ ok: true, photos });
}
